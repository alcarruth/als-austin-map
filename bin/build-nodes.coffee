...
class CoffeeScript
  ...
  make: (dest = 'dist/') =>
    ...
    fs.copyFileSync(dest + '/' + path, 'src/' + path)
    @js_min.make(dest)
...
class JavaScript
  ...
  constructor: (@name, @dir) ->
    ...
    @js_min = new Apply(@name, minify, @js, @dir, 'min.js', 'js')
...
class QueryURL
  ...
  toString: =>
    "QueryURL(#{@base_url}, #{@query}, #{@options})\n"
...
class HTML_Minified_Page
  ...
  toString: =>
    "HTML_Minified_Page(#{@name}, #{@template}, #{@data})\n"
